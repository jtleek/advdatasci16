...
You are required to pick one of the four data analysis options below or submit a proposal (no later than the 3rd day of class) petitioning to do a different project. We will let you know before the 4th day of class whether you have approval. If you are looking for other ideas consider these resources: 
...
<<<<<<< HEAD
You can install the pre-course swirl module with the command:
=======
### Set up your Rprofile

In order to track your progress in `swirl`, you need to create a directory.  For example, `~/swirl_classes`.  Then you must add these 2 lines to `~/.Rprofile` (if that file does not exist, you must create it):

```{r, eval = FALSE, echo = TRUE}
options(
  swirl_data_dir = "/path/to/progress",
  swirl_user = "my_jhed_id"
)
```
where you change the path and JHED information.  

If you start `R`, then you should be able to see if this is correct using:
...
You can install the course swirl modules with the commands:
>>>>>>> 3ddda5250023e90360082cdd5a0ea187181bc021
...
Throughout the rest of the course we will be using the modules in the swirl course specifically designed for this class. At the end of class we will point to the module you need to install and you will have until the next class period to go through the swirl module. 
...
| week/lecture  | lecture | swirl assignment |
|:------------|:------------|:------------|
| 0/1 | [Welcome]() | |
|---|---|---|
