...
Load the package and start a session. You are going to have to install the python tensorflow package first. 
...
Set up the placeholders for the data you are going to use to train
...
Create functions for generating weights and biases
...
Set up the pooling and convolutional functions
...
Reshape images with dimensions for color channels
...
Convolve with weights, add bias, then max pool
...
Reshape to have a densly connected layer
...
Apply a logistic ("softmax") function
...
Now set up the functino you are optimizing
