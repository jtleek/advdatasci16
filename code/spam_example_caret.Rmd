...
trainCapAveS <- predict(preObj,training[,-59])$capitalAve
mean(trainCapAveS)
sd(trainCapAveS)
```
...
```{r }
testCapAveS <- predict(preObj,testing[,-59])$capitalAve
mean(testCapAveS)
sd(testCapAveS)
```
...
```{r }
preObj <- preProcess(training[,-59],method=c("BoxCox"))
trainCapAveS <- predict(preObj,training[,-59])$capitalAve
...
```{r }
preProc <- preProcess(log10(spam[,-59]+1),method="pca",pcaComp=2)
spamPC <- predict(preProc,log10(spam[,-59]+1))
...
```{r }
preProc <- preProcess(log10(training[,-59]+1),method="pca",pcaComp=2)
trainPC <- predict(preProc,log10(training[,-59]+1))
